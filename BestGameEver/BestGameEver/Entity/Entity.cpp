...
void Entity::AddComponent(ComponentVec comps) {
	for (int i = 0; i < comps.size(); i++) {
		std::string temp = componentIDMap.find(comps[i]->GetComponentID())->second;
...
void Entity::RemoveComponent(ComponentID id) {
	std::string temp = componentIDMap.find(id)->second;
...
void Entity::SendMessage(ComponentMessage &msg) {
	std::string temp = componentIDMap.find(msg.compID)->second;
